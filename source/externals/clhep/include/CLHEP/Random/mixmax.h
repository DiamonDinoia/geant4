...
class MixMaxEngine {
   public:
     ...
     */
    MIXMAX_HOST_AND_DEVICE
    MixMaxEngine(std::uint64_t seed) {
        if (seed == 0) {
            seedZero();
            return;
        }
#ifndef __CUDA_ARCH__
        unpackAndBigSkip(0, seed);
#else
     ...
     */
    MIXMAX_HOST_AND_DEVICE
    MixMaxEngine(std::uint32_t clusterID, std::uint32_t machineID, std::uint32_t runID, std::uint32_t streamID) {
     ...
     */
    MIXMAX_HOST_AND_DEVICE
    MixMaxEngine(std::uint64_t seed, std::uint64_t stream) { unpackAndBigSkip(seed, stream); }
    ...
    MIXMAX_HOST_AND_DEVICE
    MIXMAX_CONSTEXPR MixMaxEngine(const uint64_t* state, uint64_t sum_over_new, uint32_t counter)
        : m_SumOverNew(sum_over_new), m_Counter(counter) {
        for (auto i = 0; i < N; ++i) { m_State[i] = state[i]; }
    }

    MIXMAX_HOST_AND_DEVICE
    constexpr MixMaxEngine(MixMaxEngine const& other) = default;
    MIXMAX_HOST_AND_DEVICE
    constexpr MixMaxEngine(MixMaxEngine&& other) noexcept = default;
    MIXMAX_HOST_AND_DEVICE
    MIXMAX_CONSTEXPR MixMaxEngine& operator=(MixMaxEngine&& other) noexcept = default;
    MIXMAX_HOST_AND_DEVICE
...
private:
   ...
   public:
#ifndef __CUDA_ARCH__
    friend std::ostream& operator<<(std::ostream& os, const MixMaxEngine& rng) {
        os << "V: ";
        for (const auto& elem : rng.m_State) { os << elem << " "; }
        os << " counter: " << static_cast<u_int32_t>(rng.m_counter) << " SumOverNew: " << rng.m_SumOverNew;
        return os;
    }
    friend std::istream& operator>>(std::istream& is, MixMaxEngine& rng) {
        for (auto& elem : rng.m_State) { is >> elem; }
        is >> rng.m_Counter >> rng.m_SumOverNew;
        return is;
    }
...
using MixMaxRng8   = internal::MixMaxEngine<internal::SMALL>;
using MixMaxRng17  = internal::MixMaxEngine<internal::MEDIUM>;
using MixMaxRng240 = internal::MixMaxEngine<internal::LARGE>;
